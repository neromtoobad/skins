...
### Reference library + extraction pipeline
- `src/references/deep-references.ts` — the curated **deep reference library**: full design blueprints (palette, type, motion, section-by-section, signature). `generate_brief` prefers a strong match here (score ≥ 3) over the 61 thin `motionsites-data.ts` summaries. The `// ADD-REFERENCE-MARKER` is where new entries are inserted — do not remove it.
- **Growing the library:** `npm run add-reference -- <candidate.json>` validates a `DeepReference` and appends it (then `npx tsc --noEmit` re-validates structurally). Recipe for agents: [docs/EXTRACTION_RECIPE.md](docs/EXTRACTION_RECIPE.md). Technique ids in the candidate must match the toolkit in `src/generators/brief.ts` (kept in sync in `scripts/add-reference.mjs`).
